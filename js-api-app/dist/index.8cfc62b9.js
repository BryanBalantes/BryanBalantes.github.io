...
const searchImages = async ()=>{
    try {
        ...
        const API_URL = `https://api.unsplash.com/search/photos?page=${page}&query=${inputData}&client_id=${atob("TUQtY0pDbVJYRnBOVWUxcDdYRUpuVzl6WVlyVG9PUFJmRnZKcGV3cUVnNA==")}&per_page=12`;
        ...
        if (results.length === 0) // console.log(inputData);
        // console.log(data);
...
const sampleImage = async ()=>{
    const API_URL = `https://api.unsplash.com/search/photos?page=${page}&query=all&client_id=${atob("TUQtY0pDbVJYRnBOVWUxcDdYRUpuVzl6WVlyVG9PUFJmRnZKcGV3cUVnNA==")}&per_page=15`;
    const response = await fetch(API_URL);
    const data = await response.json();
    const results = data.results;
    if (page === 1) searchResults.innerHTML = "";
